...
export function MobileLiveSessionDetailPage() {
  ...
  if (isLoading) {
    return <MobileLoading text="鍔犺浇鐩存挱璇︽儏..." />
  }

  if (error || !session) {
    return (
      <div>
        <MobilePageHeader title="鐩存挱璇︽儏" showBack onBack={() => navigate(-1)} />
        <MobileEmptyState
          icon="鉂?
          title="鐩存挱涓嶅瓨鍦?
      ...
      <div className="flex-1">
          ...
          <div className="aspect-video flex items-center justify-center">
            {isLive ? (
              <div className="text-white text-center">
                <div className="text-5xl mb-3 animate-pulse">馃敶</div>
                <p className="text-base">鐩存挱杩涜涓?/p>
              </div>
            ) : isScheduled ? (
              <div className="text-white text-center">
                <div className="text-5xl mb-3">鈴?/div>
                <p className="text-base">鍗冲皢寮€濮?/p>
              ...
              <div className="text-white text-center">
                <div className="text-5xl mb-3">馃摷</div>
                <p className="text-base">鐩存挱宸茬粨鏉?/p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="p-4">
          ...
          <div className="bg-white rounded-lg shadow p-4 mb-4">
            <h2 className="font-semibold mb-2">鐩存挱璇︽儏</h2>
            <p className="text-gray-600 text-sm">{session.description || '鏆傛棤璇︽儏鎻忚堪'}</p>
          </div>

          <div className="bg-white rounded-lg shadow p-4 mb-4">
            <h2 className="font-semibold mb-2">鏃堕棿瀹夋帓</h2>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">缁撴潫鏃堕棿</span>
                <span>{new Date(session.scheduledEndAt).toLocaleString()}</span>
              </div>
              {session.actualStartAt && (
                <div className="flex justify-between">
                  <span className="text-gray-500">瀹為檯寮€濮?/span>
