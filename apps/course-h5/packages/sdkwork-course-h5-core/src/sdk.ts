...
export function createCourseSdk(config: CourseSdkConfig) {
  return {
    ...
    liveSessions: {
      list: async () => {
        const url = new URL(`${config.appApi.prefix}/course_live_sessions`, config.appApi.baseUrl)
        const response = await fetch(url.toString())
        return response.json()
      },
      retrieve: async (liveSessionId: string) => {
        const url = new URL(`${config.appApi.prefix}/course_live_sessions/${liveSessionId}`, config.appApi.baseUrl)
        const response = await fetch(url.toString())
        return response.json()
      },
      join: async (liveSessionId: string) => {
        const url = new URL(`${config.appApi.prefix}/course_live_sessions/${liveSessionId}/join`, config.appApi.baseUrl)
